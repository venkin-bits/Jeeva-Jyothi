...
export function Header({ variant }: HeaderProps) {
  ...
  if (variant === "simple") {
    // PLACEHOLDER: simple variant (About/Programs/Documents/Gallery pages) —
    // single always-sticky row, no utility bar. Not wired up in this chunk
    // per BUILD_BRIEF_01 scope. Build in the chunk that covers those pages,
    // reusing the nav markup pattern below rather than rewriting Header.
    return (
      <header className="sticky top-0 z-40 bg-cream border-b border-violet-300/40">
        {/* PLACEHOLDER: simple single-row nav — build in later chunk */}
      </header>
    );
  }
